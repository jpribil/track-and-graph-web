...
function GroupForm({ onCreated, setError }: { onCreated: (group: Group) => void; setError: (error: string) => void }) {
  ...
  const [busy, setBusy] = useState(false);
  const submit = async (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); setBusy(true); const formElement = event.currentTarget; const form = new FormData(formElement); try { const response = await api<{ group: Group }>("/api/groups", { method: "POST", body: JSON.stringify({ name: form.get("name"), description: form.get("description") }) }); formElement.reset(); setOpen(false); onCreated(response.group); } catch (cause) { setError((cause as Error).message); } finally { setBusy(false); } };
...
function GroupView({ groupDetail, onTracker, onTrackerCreated, setError }: { groupDetail: GroupDetail; onTracker: (id: string) => void; onTrackerCreated: () => void; setError: (error: string) => void }) {
  ...
  return <div className="content"><header className="page-header"><div><p className="eyebrow">{t("group")}</p><h1>{group.name}</h1>{group.description && <p className="muted">{group.description}</p>}</div><div className="header-actions"><CsvImportButton groupId={group.id} onImported={onTrackerCreated} setError={setError} /><a className="secondary-button" href={`/api/groups/${group.id}/export.csv`}>{t("exportCsv")}</a><button className="primary-button compact" onClick={() => setOpen(true)}>+ {t("newTracker")}</button></div></header>{open && <TrackerForm groupId={group.id} onCreated={() => { setOpen(false); onTrackerCreated(); }} setError={setError} />}{trackers.length === 0 ? <div className="empty-panel"><h2>{t("nothingHere")}</h2><p>{t("trackerExplain")}</p><button className="primary-button" onClick={() => setOpen(true)}>{t("createTracker")}</button></div> : <div className="tracker-grid">{trackers.map((tracker) => <button className="tracker-card" key={tracker.id} onClick={() => void onTracker(tracker.id)}><span className="tracker-card-top"><span className="tracker-icon">⌁</span><span>{tracker.point_count} {t("records")}</span></span><strong>{tracker.name}</strong><span className="tracker-total">{formatValue(tracker.total, tracker.is_duration, language)}</span><span className="tracker-card-bottom">{t("total")} <i>→</i></span></button>)}</div>}</div>;
}

function CsvImportButton({ groupId, onImported, setError }: { groupId: string; onImported: () => void; setError: (error: string) => void }) {
  const { t } = useI18n();
  const [busy, setBusy] = useState(false);
  const upload = async (file: File | undefined) => { if (!file) return; setBusy(true); try { const response = await fetch(`/api/groups/${groupId}/import.csv`, { method: "POST", body: (() => { const form = new FormData(); form.append("file", file); return form; })() }); const result = await response.json(); if (!response.ok) throw new Error(result.error); onImported(); } catch (error) { setError((error as Error).message); } finally { setBusy(false); } };
  return <label className="secondary-button">{busy ? t("working") : "Import CSV"}<input type="file" accept=".csv,text/csv" hidden onChange={(event) => void upload(event.target.files?.[0])} /></label>;
}
...
function TrackerView({ detail, onBack, onPointCreated, setError }: { detail: TrackerDetail; onBack: () => void; onPointCreated: () => void; setError: (error: string) => void }) {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); setBusy(true); const formElement = event.currentTarget; const form = new FormData(formElement); try { await api(`/api/trackers/${detail.tracker.id}/points`, { method: "POST", body: JSON.stringify({ value: form.get("value"), label: form.get("label"), note: form.get("note"), trackedAt: new Date(when).toISOString() }) }); formElement.reset(); setWhen(dateTimeLocalValue()); onPointCreated(); } catch (cause) { setError((cause as Error).message); } finally { setBusy(false); } };
