...
app.post("/api/auth/register", async (request, reply) => {
  if (!allowRegistration) return reply.code(403).send({ error: "Registrace je správcem vypnutá." });
  const body = request.body as { email?: unknown; password?: unknown };
  const email = text(body?.email)?.toLowerCase();
  const password = typeof body?.password === "string" ? body.password : "";
  if (!email || !email.includes("@") || password.length < 10) {
    return reply.code(400).send({ error: "Zadej platný e-mail a heslo alespoň o 10 znacích." });
  }
  try {
    const id = randomUUID();
    ...
    if ((error as { code?: string }).code === "23505") return reply.code(409).send({ error: "Účet s tímto e-mailem již existuje." });
    throw error;
  }
});
...
app.post("/api/auth/login", async (request, reply) => {
  ...
  const email = text(body?.email)?.toLowerCase();
  const password = typeof body?.password === "string" ? body.password : "";
  if (!email || !password) return reply.code(400).send({ error: "Zadej e-mail i heslo." });
  ...
  if (!user || !(await verifyPassword(password, user.password_hash))) {
    return reply.code(401).send({ error: "Neplatný e-mail nebo heslo." });
  }
...
app.post("/api/groups", async (request, reply) => {
  ...
  if (!name) return reply.code(400).send({ error: "Skupina potřebuje název." });
  const id = randomUUID();
  await db.query(
...
app.get("/api/groups/:groupId", async (request, reply) => {
  ...
  if (!(await ownsGroup(user.id, groupId))) return reply.code(404).send({ error: "Skupina nebyla nalezena." });
...
app.post("/api/groups/:groupId/trackers", async (request, reply) => {
  ...
  if (!name || (body?.defaultValue !== "" && body?.defaultValue !== undefined && defaultValue === null)) {
    return reply.code(400).send({ error: "Tracker potřebuje název a platnou výchozí hodnotu." });
  }
  const id = randomUUID();
  const description = typeof body?.description === "string" ? body.description.trim().slice(0, 500) : "";
  await db.query(
...
app.get("/api/trackers/:trackerId", async (request, reply) => {
  ...
  if (!(await ownsTracker(user.id, trackerId))) return reply.code(404).send({ error: "Tracker nebyl nalezen." });
...
app.post("/api/trackers/:trackerId/points", async (request, reply) => {
  ...
  if (value === null || Number.isNaN(trackedAt.valueOf())) return reply.code(400).send({ error: "Zadej platnou hodnotu a čas." });
...
app.setNotFoundHandler((request, reply) => {
  if (request.raw.url?.startsWith("/api/")) return reply.code(404).send({ error: "Nenalezeno." });
